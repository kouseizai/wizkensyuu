...
export default function KioskPage() {
  ...
  const punch = (type: PunchType) => {
    ...
    setDone({
      type,
      time: `${String(now2.getHours()).padStart(2, "0")}:${String(
        now2.getMinutes()
      ).padStart(2, "0")}`,
    });
    setPin("");
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 pb-12">
        ...
        <div className="grid w-full max-w-3xl gap-6 md:grid-cols-2">
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.06] p-6 backdrop-blur-xl">
            ...
            {method === "pin" && (
              <div>
                <div className="mb-4 flex h-12 items-center justify-center gap-2 rounded-xl bg-white/10 font-mono text-2xl tracking-[0.4em]">
                  {pin ? "•".repeat(pin.length) : <span className="text-white/40">PIN入力</span>}
                </div>
            ...
            {method === "ic" && (
              <div className="flex flex-col items-center justify-center gap-4 py-10 text-center">
                <motion.div
                  ...
                  className="flex h-24 w-24 items-center justify-center rounded-3xl bg-white/10"
                >
                  <CreditCard size={44} />
                </motion.div>
                <p className="text-white/70">ICカードをかざしてください</p>
              </div>
            )}

            {method === "qr" && (
              <div className="flex flex-col items-center justify-center gap-4 py-8 text-center">
                <div className="grid h-32 w-32 grid-cols-5 grid-rows-5 gap-1 rounded-2xl bg-white p-3">
                  {Array.from({ length: 25 }).map((_, i) => (
                    <span
                      key={i}
                      className={`rounded-sm ${
                        [0, 1, 4, 5, 6, 9, 12, 14, 18, 20, 22, 24].includes(i)
                          ? "bg-[#11244a]"
                          : "bg-transparent"
                      }`}
                    />
                ...
                <p className="text-white/70">アプリでQRを読み取ってください</p>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3">
            <PunchBtn
              ...
              disabled={status !== "before" && status !== "done"}
              icon={<LogIn size={26} />}
              label="出勤"
...
function PunchBtn({
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      className={`flex ${
        small ? "h-28 flex-col" : "h-24 flex-row"
      } items-center justify-center gap-2 rounded-[24px] bg-gradient-to-br ${tone} font-bold text-white shadow-lg transition-opacity disabled:opacity-30`}
    >
      {icon}
